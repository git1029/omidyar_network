...
const vertexShader = /* glsl */ `
  ...
  void main() {
    ...
    gl_PointSize = uResolution.x * 1./uGrid * .5 * s * b.w;
    // if (aIndex == 0.) gl_PointSize *= b.w;
...
varying float id;
  void main() {
    ...
    if (id == 0.) {
      ...
      d = smoothstep(.5, .9, (1.-abs(gl_PointCoord.y * 2. - 1.) * 1.) * .9);
      // d = 0.;
      // d *= smoothstep(.5, 1., (1.-abs(gl_PointCoord.x * 2. - 1.) * .15));
    }
...
varying float id;
  void main() {
    ...
    // }
    d = smoothstep(.5, .9, (1.-abs(gl_PointCoord.y * 2. - 1.) * 1.) * .9);
      d *= smoothstep(0., .5, 1.-abs(gl_PointCoord.x * 2. - 1.));
    gl_FragColor = vec4(vec3(1.),d);
  }
`;
  ...
  const [position, index] = useMemo(() => {
    ...
    for (let j = 0; j < 1; j++) {
      for (let i = 0; i < count; i++) {
        ...
        const x = (i % grid) / grid - 0.5 + 0.5 / grid;
...
const PatternSVG = () => {
  ...
  const target2 = useFBO(512 * 1, 512 * 1);
  ...
  return (
    <>
      ...
      {createPortal(<PatternScene2 />, scene2)}
      <mesh scale={[1, 1, 1]}>
        <planeGeometry args={[1, 1]} />
        <shaderMaterial
          vertexShader={
        ...
        uniform float uGrid;
            ...
            void main() {
              ...
              float Size = 24.0; // BLUR SIZE (Radius)
              // GAUSSIAN BLUR SETTINGS }}}
              ...
              Color.rgb = smoothstep(.75, .85, Color.rgb);


              vec4 c1 = texture(uImage, vUv);
              vec4 c2 = texture(uImage2, vUv);

              // vec4 cc = vec4(clamp(c1.rgb * c1.a + c2.rgb * c2.a, 0., 1.), c1.a + c2.a);

              // vec4 cc = vec4(mix(vec3(0.), vec3(1.), clamp(smoothstep(.0, .5, c1.a) + smoothstep(0., .75, c2.a), 0., 1.)), 1.);
              // vec4 cc = vec4(mix(vec3(0.), vec3(1.), clamp(c1.a + c2.a, 0., 1.)), 1.);
              vec4 cc = pow(c1 + c2, vec4(.5));

              cc = smoothstep(.5, 1., cc);
          ...
          uniforms={{
            ...
            uImage2: new Uniform(target2.texture),
          }}
        />
      </mesh>
    </>
