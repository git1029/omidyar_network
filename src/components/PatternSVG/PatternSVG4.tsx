...
const vertexShader = /* glsl */ `
  ...
  void main() {
    ...
    gl_PointSize = uResolution.x * 1./uGrid * .5 * s * .1;
...
varying float id;
  void main() {
    ...
    // } 
    // else if (id == 1.) {
    //   d = step(.75, (1.-abs(gl_PointCoord.y * 2. - 1.) * 1.) * .9);
    //   // d = 0.;
    //   // d *= smoothstep(.5, 1., (1.-abs(gl_PointCoord.x * 2. - 1.) * .15));
    // }
    float d = distance(gl_PointCoord, vec2(0.5));
    gl_FragColor = vec4(vec3(1.-smoothstep(.4, 0.5, d)) * vec3(1.,0.,0.),1.);
  }
`;
...
const PatternSVG = () => {
  ...
  useEffect(() => {
    ...
    if (points.current) {
      points.current.geometry.setIndex(null);
    }
