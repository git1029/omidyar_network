...
varying float id;
  void main() {
    ...
    if (id == 0.) {
      ...
      d *= mix(sin(gl_PointCoord.x * 3.14159), smoothstep(.0, .25, (1.-abs(gl_PointCoord.x * 2. - 1.))), 1.);
...
const PatternScene = ({ grid }: { grid: number }) => {
  ...
  const [position, index] = useMemo(() => {
    ...
    const index = new Float32Array(count * 1 * 2);
...
const PatternSVG = () => {
  ...
  const ctex = new CanvasTexture(offscreen);
  // const [camera] = useState(new OrthographicCamera(-1, 1, 1, 1, -1, 1));

  const target = useFBO(512 * 2, 512 * 2);
  // const target2 = useFBO(512 * 2, 512 * 2);

  useFrame(() => {
    if (plane2.current) {
      plane2.current.visible = false;
      // plane2.current.visible = true;
    }
    if (plane.current) {
    ...
    if (glo) {
      glo.clearRect(0, 0, 512, 512);
      glo.drawImage(gl.domElement, 0, 0);
    ...
    // if (plane.current) {
    //   plane.current.material.uniforms.uImage.value = ctex;
    // }

    // if (plane.current) {
    //   plane.current.visible = false;
    //   // plane2.current.material.map.needsUpdate = true;
    // }
    // if (plane2.current) {
    //   plane2.current.visible = true;
    //   plane2.current.material.map = ctex;
    //   plane2.current.material.map.needsUpdate = true;
    // }
  ...
  return (
    <>
      ...
      <mesh ref={plane2} position={[0, 0, 0.1]}>
        <planeGeometry args={[1, 1]} />
        <meshBasicMaterial color={0xff0000} map={ctex} />
      </mesh>
    </>
  );
};
