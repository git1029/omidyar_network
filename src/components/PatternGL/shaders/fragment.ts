...
float brightness(vec3 color) {
  ...
  void main() {
    ...
    for (float i = -1.; i <= 1.; i++) {
      ...
      float x_ = x + i;
      float y_ = y + j;
      if (x_ < 0. || x_ > grid - 1. || y_ < 0. || y_ > grid - 1.) continue;
    ...
    // t = 1.;
    float r2 = 1./grid * 1. * mix(1., 2., mod(ii, 2.)) * mix(.5, .75, t);
    // float r2 = 1./grid * 1.5 * mix(.5, 1., pow(1.-b, 1.));

      // if (l < r * 2.) d = min(d, sdCircle(uv - p - .5/grid, r / 4.));
      // d = smoothUnionSDF(d, sdCircle(uv - p - .5/grid, r / 4.), 0.01);
        // uv.y -= .5;
        // uv.y *= scl;
        // uv.y += .5;

      d1 = smoothUnionSDF(d1, sdCircle(uv * vec2(scl) - p - .5/grid * 0., r2 / 4.), 0.02);
      // d = smoothUnionSDF(d, sdCircle(uv - p - .5/grid + vec2(.25/grid, 0.), r / 8.), .0);
      // if (x_ < grid - 1.) d = smoothUnionSDF(d, sdBox(uv - p - .5/grid - vec2(.5/grid, 0.), vec2(.25/ grid*1.5, .125/grid*.5)), 0.1);

  // vec2 t = uv;
  // t *= vec2(2., 1.);
  // t += (- p - .5/grid - vec2(.5/grid, 0.)) * vec2(2., 1.);

      // if (x_ < grid - 1.) d2 = min(d2, sdRoundedBox(uv - p - .5/grid - vec2(.5/grid, 0.)* sin(uTime * PI), vec2(.2/grid, .125/grid*.5), vec4(.125/grid*.5)));
      ...
      // Vertical
      vec2 s1 = vec2(s0.y, s0.x) * scl;
      vec2 off1 = vec2(0., s1.y/1.);
      vec2 q2 = q - off1;
      vec2 q3 = q + off1;
      if (y_ < grid - 1.) d2 = min(d2, sdRoundedBox(q2, s1, round));
      if (y_ > 0.) d2 = min(d2, sdRoundedBox(q3, s1, round));
      
      // if (x_ > 0.) d2 = min(d2, sdRoundedBox(uv - p - .5/grid + vec2(.5/grid, 0.)* sin(uTime * PI), vec2(.2/grid, .125/grid*.5), vec4(.125/grid*.5)));
      // if (x_ > 0.) d = smoothUnionSDF(d, sdBox(uv - p - .5/grid + vec2(.5/grid, 0.), vec2(.4/grid, .125/grid*.25)), 0.02);
      // if (x_ > 0.) d = smoothUnionSDF(d, sdBox(uv - p - .5/grid + vec2(r/2.25, 0.), vec2(r/4., .125/grid*.5)), 0.1);
      // d = smoothUnionSDF(d, sdCircle(uv - p - .5/grid, r / 4.), r/1.25);
    }
  }


  // float scl = 1./sqrt(3.);
  // float angle = angleBetween(vec2(.5, .5), vec2(1., 1.5));
