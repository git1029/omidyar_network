  ...
  void main() {
    ...
    for (float i = -1.; i <= 1.; i++) {
      for (float j = -1.; j <= 1.; j++) {
        ...
        // // #ISOMETRIC
        // if (uGrid == 1. && uConnectors.x == 1.) d1 = getNodeIso(uv, i, j, id, sbox, af, ascl, true, roundness, threshold, d1, b0.w, grid, diff, logop);
        // if (uGrid == 1. && uConnectors.y == 1.) d1 = getNodeIso(uv, i, j, id, sbox, af, ascl, false, roundness, threshold, d1, b0.w, grid, diff, logop);
      }
    }
